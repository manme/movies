  ...
  class << self
    ...
    def all_categories_with_count
      ActsAsTaggableOn::Tagging.
        joins(:tag).joins("INNER JOIN movies ON movies.id = taggings.taggable_id AND taggings.taggable_type = 'Movie'").
    ...
    def all_scores_with_count
      (1..Movie::SCORE_MAX).to_a.inject({}) do |mem, score|
    ...
    def categories_for(movie_ids)
      ...
      movies_categories.inject(Hash.new{ |h,k| h[k]=[] }) do |h,(k,v)|
        h[k] << v
        h
      end
    end
